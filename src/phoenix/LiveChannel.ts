...
export class LiveChannel {
  ...
  join(): void {
    if (!this.joined) {
      this.channel.onError((e: any) => this.emitError('channel error', e));
      this.channel
        .join()
        .receive('ok', (resp: any) => {
      ...
      this.channel.on('state:change', (state: any) => this.handleChange(state));
      this.channel.on('state:patch', (patch: any) => this.handlePatch(patch));
      this.channel.on('error', (error: any) => this.emitServerError(error));
      this.joined = true;
    }
  }
   ...
   */
  getEventsForType$(type: string): Observable<any> {
  ...
  private emitServerError(error: any): void {
  ...
  pushEvent(eventName: string, payload: any): void {
